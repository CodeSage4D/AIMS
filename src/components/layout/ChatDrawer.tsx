...
export default function ChatDrawer({ isOpen, onClose, currentUser }: ChatDrawerProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end select-none">
      ...
      <div className="relative w-full max-w-lg h-full bg-[#0a0f1d] border-l border-white/[0.08] shadow-2xl flex flex-col z-50 animate-slideOver text-white">
        
        {/* Header */}
        <div className="p-4 border-b border-white/[0.08] flex items-center justify-between bg-[#0e162a]">
          <div className="flex items-center space-x-2.5">
            <MessageSquare className="h-5 w-5 text-indigo-400" />
            <div>
              <h3 className="text-sm font-extrabold font-heading">AIMS Messaging Center</h3>
              <p className="text-[10px] text-gray-400">Direct workspace support and team channels.</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          {/* Contacts Column */}
          <div className="w-1/3 border-r border-white/[0.08] flex flex-col bg-[#080d19]">
            ...
            <div className="flex-1 overflow-y-auto p-2 space-y-1.5">
              <span className="text-[9px] font-heading font-extrabold text-gray-500 uppercase tracking-widest px-3 block select-none">
                Workspace Contacts
              </span>
              ...
              {contactsLoading ? (
                ...
                <div className="text-[10px] text-center text-gray-500 font-bold py-6 px-2">
                  No online contacts
                </div>
              ) : (
                contacts.map((contact) => (
                  ...
                  >
                    ...
                    <span className="text-[8.5px] uppercase font-heading font-bold text-gray-400 tracking-wider truncate">
          ...
          {/* Active Chat Column */}
          <div className="flex-1 flex flex-col bg-[#090e1c]">
            {/* Active Contact Header Banner */}
            <div className="px-4 py-3 bg-[#0d1428] border-b border-white/[0.08] flex items-center justify-between">
              <div className="flex items-center space-x-2.5 min-w-0">
                <div className="h-7 w-7 rounded-full bg-primary/10 border border-primary/20 flex items-center justify-center text-xs font-heading font-extrabold text-indigo-400 select-none">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-[9px] text-gray-400 font-medium truncate uppercase tracking-wide">
                    {activeContact
              ...
              {activeContact && (
                <a
                  ...
                  className="flex items-center space-x-1.5 px-2.5 py-1.5 rounded-lg bg-indigo-600/10 hover:bg-indigo-600/20 border border-indigo-500/25 text-indigo-400 text-[10px] font-heading font-extrabold uppercase transition-all shrink-0 cursor-pointer"
              ...
              {messagesLoading && messages.length === 0 ? (
                ...
                messages.map((msg) => {
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center space-x-1.5 text-[9px] font-bold text-gray-400">
                        <span>{msg.sender.fullName}</span>
                        <span className="text-[8px] uppercase px-1 rounded bg-secondary font-heading shrink-0 tracking-wide font-extrabold">
                          {msg.sender.role}
                        </span>
                      </div>
                      <div
                        className={cn(
                          "px-3.5 py-2.5 rounded-2xl text-xs font-medium leading-relaxed shadow-md border",
                          isMe
                            ? "bg-indigo-600/10 border-indigo-500/20 text-white rounded-tr-none"
                            : "bg-[#131b31] border-white/[0.06] text-gray-100 rounded-tl-none"
                        )}
                      >
                        {msg.content}
                      </div>
            ...
            <form onSubmit={handleSendMessage} className="p-3 bg-[#0c1223] border-t border-white/[0.08] flex items-center space-x-2">
              <input
                type="text"
                value={newMessage}
                onChange={(e) => setNewMessage(e.target.value)}
                placeholder="Type your message securely..."
                className="flex-1 h-10 px-3 bg-[#11172a] hover:bg-[#131b32] focus:bg-[#141d37] border border-white/[0.08] focus:border-indigo-500/50 rounded-xl text-xs font-medium placeholder:text-gray-500 focus:outline-none focus:ring-1 focus:ring-indigo-500/25 transition-all text-white"
              />
              <button
                type="submit"
                ...
                className={cn(
                  ...
                  (!newMessage.trim() || sending) && "opacity-50 cursor-not-allowed bg-indigo-600/40"
                )}
              >
                {sending ? (
